...
struct fuzzy *fuzzy_init(unsigned int input_num, unsigned int output_num) {
    struct fuzzy *fuzzy_struct = (struct fuzzy *) pvPortMalloc(sizeof(struct fuzzy));
    fuzzy_struct->input_num = input_num;
    fuzzy_struct->output_num = output_num;
    fuzzy_struct->mf_type = (unsigned int *) pvPortMalloc((input_num + output_num) * sizeof(unsigned int));
#ifdef fuzzy_pid_rule_base_deep_copy
    fuzzy_struct->mf_params = (int *) pvPortMalloc(4 * qf_default * sizeof(int));
    fuzzy_struct->rule_base = (int *) pvPortMalloc(output_num * qf_default * qf_default * sizeof(int));
#endif
    fuzzy_struct->output = (float *) pvPortMalloc(output_num * sizeof(float));
    return fuzzy_struct;
}

void delete_fuzzy(struct fuzzy *fuzzy_struct) {
    vPortFree(fuzzy_struct->mf_type);
    vPortFree(fuzzy_struct->output);
    vPortFree(fuzzy_struct);
}
...
struct PID *raw_fuzzy_pid_init(float kp, float ki, float kd, float integral_limit, float dead_zone,
    ...
    struct PID *pid = (struct PID *) pvPortMalloc(sizeof(struct PID));
    pid->kp = kp;
    pid->ki = ki;
    pid->kd = kd;
...
void delete_pid_vector(struct PID **pid_vector, unsigned int count) {
    ...
    vPortFree(pid_vector);
}

struct PID **pid_vector_init(float params[][pid_params_count], unsigned int count) {
    struct PID **pid = (struct PID **) pvPortMalloc(sizeof(struct PID *) * count);
    for (unsigned int i = 0; i < count; ++i) {
